...
The Photofield MCP ([Model Context Protocol](https://modelcontextprotocol.io)) server lets LLM agents search and retrieve photos from your Photofield instance through a simple, agent-friendly interface that avoids manual browser navigation and raw API calls. It runs on the same port as the main server at `/mcp` by default.
