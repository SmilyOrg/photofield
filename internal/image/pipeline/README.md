...
Clean, typed, sequential batch architecture for indexing photo metadata, thumbnails, and contents (color/AI).
...
cfg := pipeline.Config{
    ...
    ThumbnailSources:    source.ThumbSources(),
    ThumbnailGenerators: source.ThumbGenerators(),
    ThumbnailSink:       source.ThumbSink(),
...
The pipeline runs in **two sequential stages**, each completing fully before the next begins:
...
Stage functions return output channels instead of accepting output parameters. This makes data dependencies explicit at the call site:
...
Errors are logged immediately via `log.Printf()` rather than returned via error channels, which prevents race conditions and simplifies composition.

### No EXIF Thumbnail Extraction

Thumbnails are sourced only from:
1. SQLite cache (fast path)
2. Generated from original file using metadata orientation

EXIF-embedded thumbnails were removed — they have unreliable quality and dimensions, and the SQLite cache already covers the fast-path need.
